...
@modifications by: Urbano Alfonso Medina Martinez, KAUST, Saudi Arabia
"""
...
class CanteraCSP(CanteraThermoKinetics):
    ...
    def calc_TSR(self,**kwargs):
        ...
        if getM and get_weights == False:
            return [TSR, M]
        if getM and get_weights:
            return [TSR, M, weights]
        else:
            return TSR
    ...
    def calc_TSRindices(self,**kwargs):
        ...
        for key, value in kwargs.items():
            if (key == 'rtol'): 
                ...
                getTSR = value
            elif (key == 'multiply_by_eval_sign'): 
                multiply_by_eval_sign = value
            elif (key == 'type'): 
                if(value == 'timescale'):
                    useTPI = True
        ...
        M = findM(self.n_elements,self.stateYT(),self.evals,self.Revec,self.tau,self.f,rtol,atol)
        TSR, weights = findTSR(self.n_elements,self.rhs,self.evals,self.Revec,self.f,M)
        TSRidx = TSRindices(weights, self.evals,multiply_by_eval_sign=multiply_by_eval_sign)
        if (useTPI):
        ...
        TSRind = TSR_participation_indices(TSRidx, CSPidx,multiply_by_eval_sign=multiply_by_eval_sign)
        if getTSR:
            return TSR, TSRind
        else:
            return TSRind, TSRidx
    ...
    def calc_CSPindices(self,**kwargs):
        ...
        Smat = self.generalized_Stoich_matrix
        rvec = self.R_vector
        if getAPI: API = CSP_amplitude_participation_indices(self.Levec, Smat, rvec, new_formulation = only_pos_APIs)
    ...
    def calc_extended_TSR(self,jacobian_type='original_PyCSP_formulation',ptstep_info=False,**kwargs):
        ...
        saving_route = '/home/medinaua/cantera/flames_cantera/1D_Flames_OWN/OPUS_data_H2_Flames_Takuya/opus_extinction_data_without_velocity/PP_H2_air_phi04/Unsteady/New_Jacobian_Formulation_Validation_Folder/'
        saving_name_evalues = saving_route + 'eig_Jac_CT.txt'
        nv=len(self.Revec)
        getMext = False
        get_h = False
        print_vars = False
        ...
        for key, value in kwargs.items():
            if (key == 'rtol'): 
                ...
                print_vars = value
            elif (key == 'pos_ex_amplitude'):
                pos_ex_amplitude = value
            else:
                raise ValueError("unknown argument --> %s" %key)
        ...
        if jacobian_type =='original_PyCSP_formulation':
            #Compute the TSR_ext with the original PyCSP formulation
            Smat = self.generalized_Stoich_matrix
            rvec = self.R_vector    
            rhs_ext, h, Smat_ext, rvec_ext = add_transport(self.rhs,self.Levec,Smat,rvec,rhs_convYT,rhs_diffYT)
            #Rotate Levec and Revec for obtaining only extended positive amplitudes:
            if pos_ex_amplitude == True: 
                Revec_ext_rot,Levec_ext_rot,h_rot = evec_pos_ampl(self.Revec,self.Levec,h)
                #Save the variables:
                # save_jac_path = saving_route + 'jac_orig.txt'
                # save_evals_path = saving_route + 'jac_orig.txt'
                # save_jac_path = saving_route + 'jac_orig.txt'
                # save_jac_path = saving_route + 'jac_orig.txt'
                # np.savetxt("array.txt", array, delimiter=",")
                
                Mext = findM(self.n_elements,self.stateYT(),self.evals,Revec_ext_rot,self.tau,h_rot,rtol,atol)
                TSR_ext, weights_ext = findTSR(self.n_elements,rhs_ext,self.evals,Revec_ext_rot,h_rot,Mext,print_vars=print_vars)
            else:
                Mext = findM(self.n_elements,self.stateYT(),self.evals,self.Revec,self.tau,h,rtol,atol)
                TSR_ext, weights_ext = findTSR(self.n_elements,rhs_ext,self.evals,self.Revec,h,Mext,print_vars=print_vars)
            #findTSR(self.n_elements,rhs_ext,self.evals,self.Revec,h,Mext,print_vars=print_vars)

        elif jacobian_type =='Jacobian_transport_and_chemistry':
            #Compute Jacobian including transport and then proceed to compute 
            #TSRs with this new information. First check that the previous tstep info.
            if ptstep_info == False:
                raise ValueError("Input the previous tstep info to be able to compute the jacobian!")
            else: 
                #Now check that the information is correct.
                if len(ptstep_info) != 3:
                    raise ValueError("Check the shape of the ptstep information! It should be a list of 3 lists, in the following order: rhs_chem,rhs_trans,stateYT")
                elif len(ptstep_info[0]) != nv or len(ptstep_info[1]) != nv or len(ptstep_info[2]) != nv:
                    raise ValueError("Check that the number of elements in each of the lists match NSP+1")
                else:
                    #Previous timestep information is correct, proceed to compute the jacobian
                    rhs_transport = rhs_convYT + rhs_diffYT
                    self.jacobian_transport_chemistry = self.jacobian_transport_and_chemistry(ptstep_info,rhs_transport)
                    #Solve the eigen system with this new jacobian
                    self.evals_Jac_CT,self.Revec_Jac_CT,self.Levec_Jac_CT = eigsys(self.jacobian_transport_chemistry)
                    #Recompute the timescales
                    self.tau_Jac_CT = timescales(self.evals_Jac_CT)
                    # #Save the evalues:
                    # with open(saving_name_evalues, "a") as f:
                    #     f.write(",".join([str(element) for element in self.evals_Jac_CT]) + "\n")
                    #     f.write("----------------"+"\n")
                    # f.close()
                    #Add the transport 
                    Smat = self.generalized_Stoich_matrix
                    rvec = self.R_vector    
                    rhs_ext, h, Smat_ext, rvec_ext = add_transport(self.rhs,self.Levec_Jac_CT,Smat,rvec,rhs_convYT,rhs_diffYT)
                    #Find exhausted modes and compute TSR.
                    Mext = findM(self.n_elements,self.stateYT(),self.evals_Jac_CT,self.Revec_Jac_CT,self.tau_Jac_CT,h,rtol,atol)
                    TSR_ext, self.weights_ext = findTSR(self.n_elements,rhs_ext,self.evals_Jac_CT,self.Revec_Jac_CT,h,Mext)
        elif jacobian_type =='TSR_ext_transport_and_chemistry_double_scalar_form':
            rhs_transport = rhs_convYT + rhs_diffYT
            rhs_total = rhs_transport + self.rhs_const_p()
            norm_rhs_squared = np.linalg.norm(rhs_total)**2
            self.jacobian_transport_chemistry = self.jacobian_transport_and_chemistry(ptstep_info,rhs_transport)
            TSR_ext = (np.dot(rhs_total,np.matmul(self.jacobian_transport_chemistry,rhs_total)))/norm_rhs_squared
        else: 
            raise ValueError("unknown Jacobian Type --> %s" %jacobian_type)
        #Function return:
        if getMext and get_h == False and get_weights == False:
            return [TSR_ext, Mext]
        elif  get_weights and get_h == False and getMext == False: 
            return [TSR_ext, weights_ext]
        elif getMext and get_weights and  get_h == False: 
            return [TSR_ext, Mext, weights_ext]
        elif getMext and get_h and get_weights == False:
            return [TSR_ext, Mext,h]
        elif getMext and get_h and get_weights:
            if pos_ex_amplitude == True:
                return [TSR_ext, Mext,h_rot,weights_ext]
            else:
                return [TSR_ext, Mext,h,weights_ext]
        else:
            return TSR_ext
    ...
    def calc_extended_TSRindices(self,**kwargs):
        ...
        for key, value in kwargs.items():
            if (key == 'rtol'): 
                ...
                rhs_diffYT = value
            elif (key == 'only_pos_CSP_APIs'):
                only_pos_CSP_APIs = value
            elif (key == 'pos_ex_amplitude'):
                pos_ex_amplitude = value
            elif (key == 'get_TSR_CSP_PI'):
                get_TSR_CSP_PI = value
            elif (key == 'multiply_by_eval_sign'):
                multiply_by_eval_sign = value
            else:
                raise ValueError("unknown argument --> %s" %key)
        ...
        if pos_ex_amplitude ==True:
            Revec_ext_rot,Levec_ext_rot,h_rot = evec_pos_ampl(self.Revec,self.Levec,h)
            Mext = findM(self.n_elements,self.stateYT(),self.evals,Revec_ext_rot,self.tau,h_rot,rtol,atol)
            TSR_ext, weights_ext = findTSR(self.n_elements,rhs_ext,self.evals,Revec_ext_rot,h_rot,Mext)
            TSRidx = TSRindices(weights_ext, self.evals,multiply_by_eval_sign=multiply_by_eval_sign)
            #Giving as an input rotated eigenvectors and (+) amplitude
            CSPidx = CSP_amplitude_participation_indices(Levec_ext_rot, Smat_ext, rvec_ext, new_formulation = only_pos_CSP_APIs)
        else:
            Mext = findM(self.n_elements,self.stateYT(),self.evals,self.Revec,self.tau,h,rtol,atol)
            TSR_ext, weights_ext = findTSR(self.n_elements,rhs_ext,self.evals,self.Revec,h,Mext)
            TSRidx = TSRindices(weights_ext, self.evals,multiply_by_eval_sign=multiply_by_eval_sign)
            #Giving as an input rotated eigenvectors and (+) amplitude
            CSPidx = CSP_amplitude_participation_indices(self.Levec, Smat_ext, rvec_ext, new_formulation = only_pos_CSP_APIs)
            
        TSRind_ext = TSR_participation_indices(TSRidx, CSPidx,multiply_by_eval_sign=multiply_by_eval_sign)
        if getTSRext and get_API_ext == False:
            return TSR_ext, TSRind_ext
        
        elif get_API_ext and getTSRext == False and get_TSR_CSP_PI==False:
            return CSPidx, TSRind_ext
        
        elif get_API_ext and getTSRext:
            return CSPidx, getTSRext, TSRind_ext
        
        elif get_API_ext and get_TSR_CSP_PI:
            return CSPidx, TSRind_ext, TSRidx
        else:
            return TSRind_ext
    ...
    def kernel(self):
        ...
        self._jac = self.jacobian.copy() #Returns the 2D Jacobian
        #eigensystem
...
def findTSR(n_elements,rhs,evals,Revec,f,M,print_vars=False):
    n = len(Revec)
    nEl = n_elements 
    #deal with amplitudes of cmplx conjugates
    fvec = f.copy()
    imPart = evals.imag!=0 #Identify the imaginary evals, True for an imaginary element. 
    fvec_orig = fvec
    for i in range(1,n):
        if (imPart[i] and imPart[i-1] and evals[i].real==evals[i-1].real): #If the element has a imaginary evalue, and the evalues match, then you modify the i and i-1 fvec.
    ...
    if print_vars == True:
        for i in range(n):
            print(weights[i] * np.sign(evals[i].real) * np.abs(evals[i]))
        print('Amplitude Original')
        print(fvec_orig)
        print('evals')
        print(evals)  
        print('Amplitude')
...
def TSRindices(weights, evals, multiply_by_eval_sign = False):
    """Ns array containing participation index of mode i to TSR"""
    n = len(weights)
    Index = np.zeros((n))
    norm = np.sum([weights[i] * np.abs(evals[i]) for i in range(n)])
    if multiply_by_eval_sign == True: 
        for i in range(n):
            Index[i] = weights[i] * np.abs(evals[i]) * np.sign(evals[i])
...
def TSR_participation_indices(TSRidx, CSPidx,  multiply_by_eval_sign = False):
    """2Nr array containing participation index of reaction k to TSR"""
    multiply_by_eval_sign = True
    if multiply_by_eval_sign == True:
        Index = np.matmul(np.transpose(np.abs(CSPidx)),TSRidx)
        #Not sure if I should normalize or not! 
...
def eigsys(jac):        
    ...
    #sort 
    idx = np.argsort(abs(evals))[::-1] #Here you are sorting in descending order. 
    evals = evals[idx] 
    Revec = Revec[:,idx]
...
def CSPIndices(Proj, Smat, rvec, new_formulation = False):
    """Returns a Nv x 2Nr matrix of indexes, computed as Proj S r """
    ns = Smat.shape[0]
    nr = Smat.shape[1]
    Index = np.zeros((ns,nr))
    for i in range(ns):
        Proj_i = Proj[i,:] #So you take for each loop a bi vector, for which you compute the index with respect all reactions.
        Index[i,:] = CSPIndices_one_var(Proj_i, Smat, rvec, new_formulation = new_formulation)
    #np.sum(abs(Index),axis=1) check: a n-long array of ones
    return Index

def CSPIndices_one_var(Proj_i, Smat, rvec, new_formulation = False):
    ...
    if norm != 0.0:
        Index = Index/norm #So, if one of the elements of the PS vector is negative, then the index will be negative. 
    else:
        Index = np.zeros((nr))
    return Index
    
    # Old Formulation!
    # if new_formulation == True:
    #     PS = np.abs(PS)
    #     rvec = np.abs(rvec)
    # Index = np.multiply(PS,rvec)
    # norm = np.sum(abs(Index))
...
def CSP_amplitude_participation_indices(B, Smat, rvec, new_formulation = False):
    """Ns x 2Nr array containing participation index of reaction k to variable i"""
    API = CSPIndices(B, Smat, rvec, new_formulation = new_formulation)
    return API
